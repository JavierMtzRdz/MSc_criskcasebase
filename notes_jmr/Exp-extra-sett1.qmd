...
name_files <- list.files(here("paper", "results"), 
                         pattern = 'coefficients_setting-1',
           recursive = T, full.names = T)
name_model <-  tibble(name_files_comp = name_files,
                      name = str_extract(name_files, "(?<=tting-1_)(.*?)(?=.csv)"))
# walk(name_files,
...
data_proj <- name_model %>%
  ...
  separate(name, sep = "_", 
           into = c("sim", "p", "k"),
           remove = F) %>% 
  mutate(sim = as.numeric(str_remove(sim, "iter-")),
         p = as.numeric(str_remove(p, "p-")),
         k = as.numeric(str_remove(k, "k-")),
         setting = glue::glue("p = {p}, k = {k}", .sep = "=")) %>% 
  arrange(p, k)
...
vec_mse_bias <- function(.x, p, k){
    ...
    beta1[nu_ind] <- c(rep(1, k/4), rep(1, k/4), rep(1, k/4), rep(1, k/4))
    mse_bias(.x, beta1)
}

vec_varsel_perc <- function(.x, p, k){
    p <- unique(p)
    k <- unique(k)
    beta1 <- c(rep(0, p))
    beta2 <- c(rep(0, p))
...
The following plot shows the variable selection metrics such as Sensitivity, Specificity, and MCC. In this case, the proposed methods consistently achieve a higher MCC in low-dimensional scenarios, but in higher dimensions, the casebase penalized model using SVRG shows slightly lower MCC values. Still, the model with AccSVRG maintains higher MCC across all cases. It can be observed that in the high-dimensional scenario with $p = 1000$ and $k = 168$, the casebase model exhibits notably good Sensitivity, although this results from overselecting, as indicated by the Specificity. Additionally, it is worth noting that the enet casebase model using SVRG performs better than in the thesis’ results. This improvement is due to the fact that, in high dimensions, the initial implementation sometimes fails to converge and produces coefficients very close to zero but not exactly zero. To address this, the function used to estimate the selection performance metrics was modified to include a threshold where $|\beta_{k,j}| < 1 \times 10^{-6}$ are considered zero, rather than only when $|\beta_{k,j}| = 0$. This adjustment also helps accurately identify zeros affected by floating-point error.
...
coefs_mod %>% 
    group_by(name, model, setting, p, k) %>% 
    summarise(bias_mse = vec_mse_bias(coef, p, k)) %>% 
    arrange(p, k) %>% 
    filter(!str_detect(model, "post")) %>%
    ggplot(aes(x = model, y = bias_mse,
               fill = model)) +
    geom_boxplot() +
    facet_grid(~fct_inorder(setting)) +
...
coefs_mod %>% 
    group_by(name, model, setting, p, k) %>% 
    summarise(selec_mea = vec_varsel_perc(coef, p, k)) %>% 
    unnest(selec_mea) %>% 
    arrange(p, k) %>% 
    filter(!str_detect(model, "post")) %>% 
    ...
    facet_grid(fct_inorder(setting)~metric) +
    theme(axis.text.x = #element_text(angle = 90, vjust = 0.5, hjust=1)
              element_blank()) +
